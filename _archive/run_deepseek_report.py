...
def _render_decision_md(report: dict) -> str:
    as_of = report.get("as_of", "")
    title = report.get("title", f"DeepSeek Quant Report — {as_of}")
...
def main() -> None:
    ...
    out_json = reports_dir / f"deepseek_report_{as_of}.json"
    out_md = reports_dir / f"deepseek_report_{as_of}.md"

    detail_root = reports_dir / f"deepseek_details_{as_of}"
...
""",
    ...
    for idx, x in enumerate(items):
        ...
        resp_stock = _call_with_length_retry(
            ...
            max_tokens_first=STOCK_MAX_TOKENS_1,
            max_tokens_second=STOCK_MAX_TOKENS_2,
            retries=4,
            backoff_sec=3.0,
        )
...
""",
    ...
    SUM_MAX_TOKENS_1 = int(os.getenv("DEEPSEEK_SUMMARY_MAX_TOKENS", "4000"))
    SUM_MAX_TOKENS_2 = int(os.getenv("DEEPSEEK_SUMMARY_MAX_TOKENS_RETRY", "8000"))
    ...
    resp_sum = _call_with_length_retry(
        ...
        max_tokens_first=SUM_MAX_TOKENS_1,
        max_tokens_second=SUM_MAX_TOKENS_2,
        retries=5,
        backoff_sec=3.0,
    )
    ...
    # Write Markdown
    md_parts = []
    md_parts.append(_render_decision_md(report_obj))
