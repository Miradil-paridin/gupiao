...
使用方法：
    python run_make_daily_rank_v31.py
"""
...
def check_liquidity(row: pd.Series, cfg: SignalConfigV31) -> bool:
    ...
    # 20日均成交额
    amount_20d = row.get("amount_20d", row.get("amount", 0))
    if pd.notna(amount_20d) and amount_20d < cfg.min_amount_20d:
        return False

    # 20日均换手率
    turnover_20d = row.get("turnover_20d", row.get("turnover", 0))
    if pd.notna(turnover_20d) and turnover_20d < cfg.min_turnover_20d:
        return False
...
def check_eligible_strict(row: pd.Series, cfg: SignalConfigV31) -> bool:
    ...
    """
    # 趋势必须向上
    trend_up = row.get("ma_dist_20", 0) > 0
    if not trend_up:
        return False
...
def main():
    ...
    cfg = SignalConfigV31(
        ...
        pullback_min_pct=strategy.get("pullback_min_pct", 0.10),
        pullback_max_pct=strategy.get("pullback_max_pct", 0.30),
        max_single_weight=strategy.get("max_single_weight", 0.15),
        entry_mode=strategy.get("entry_mode", "strict"),
        use_industry_diversification=strategy.get("industry_diversification", {}).get("enabled", True),
        max_per_industry=strategy.get("industry_diversification", {}).get("max_per_industry", 2),
        use_liquidity_filter=strategy.get("liquidity_filter", {}).get("enabled", True),
        min_amount_20d=strategy.get("liquidity_filter", {}).get("min_amount_20d", 6e7),
        stop_loss_pct=risk.get("stop_loss_pct", 0.06),
        trailing_stop_pct=risk.get("trailing_stop_pct", 0.08),
